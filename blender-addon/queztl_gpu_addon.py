...
bl_info = {
    ...
    "doc_url": "https://github.com/queztl-core/docs",
    "category": "Render",
}
...
class QUEZTL_OT_Connect(Operator):
    ...
    bl_idname = "queztl.connect"
    bl_label = "Connect to Queztl GPU"
    bl_options = {'REGISTER'}
    
    def execute(self, context):
        props = context.scene.queztl_props
...
class QUEZTL_PT_MainPanel(Panel):
    ...
    def draw(self, context):
        ...
        if props.is_connected:
            ...
            row = box.row()
            row.operator("queztl.disconnect", icon='CANCEL')
        else:
            row.label(text="Status: Disconnected ❌", icon='ERROR')
            row = box.row()
            row.operator("queztl.connect", icon='PLUGIN')
        ...
        if props.is_connected:
            ...
            box.operator("queztl.test_render", icon='PLAY')
        ...
        if props.is_connected:
            ...
            box.operator("queztl.benchmark", icon='TIME')
        ...
        if props.is_connected:
            ...
            box.operator("queztl.get_metrics", icon='FILE_REFRESH')
...
def register():
    ...
    bpy.types.Scene.queztl_props = bpy.props.PointerProperty(type=QueztlProperties)
...
def unregister():
    ...
    del bpy.types.Scene.queztl_props

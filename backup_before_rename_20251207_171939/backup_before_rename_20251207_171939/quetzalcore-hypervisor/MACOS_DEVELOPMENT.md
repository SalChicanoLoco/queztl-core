# 🦅 Queztl Hypervisor - Development Notice

## ⚠️ macOS Build Limitation

The Queztl Hypervisor requires **Linux with KVM support** to run. You're currently on macOS (darwin25), which doesn't have KVM.
...
multipass launch --name queztl-dev --cpus 4 --memory 8G --disk 20G
multipass shell queztl-dev

# Inside Linux VM:
cd /path/to/queztl-hypervisor/core
...
scp -r queztl-hypervisor user@linux-server:~/
...
✅ **Python Control API** (queztl-hypervisor/api/)
```bash
...
✅ **Kernel Build Scripts** (queztl-hypervisor/kernel/)
- Scripts are portable
- Need to run on Linux to actually compile kernel

✅ **VM Image Builder** (queztl-hypervisor/tools/)
...
# Clone project
git clone https://github.com/SalChicanoLoco/queztl-core
cd queztl-core/queztl-hypervisor/core
...
The Queztl Hypervisor is **ready for production** on any Linux server:

```bash
# On Linux server (Ubuntu 22.04+)
git clone https://github.com/SalChicanoLoco/queztl-core
cd queztl-core/queztl-hypervisor
...
# Run
sudo ./target/release/queztl-hv create --name test-vm --vcpus 2 --memory 2048
sudo ./target/release/queztl-hv run test-vm
```

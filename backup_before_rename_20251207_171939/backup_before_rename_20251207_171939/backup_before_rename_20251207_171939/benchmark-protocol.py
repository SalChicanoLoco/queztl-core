...
async def benchmark_quetzalcore(num_requests: int):
...
def print_comparison(quetzalcore_stats, rest_stats):
    ...
    print("\n⚡ QUEZTL PROTOCOL:")
    print(f"  Average Latency:     {quetzalcore_stats['avg_latency']:.2f} ms")
    print(f"  Min Latency:         {quetzalcore_stats['min_latency']:.2f} ms")
    print(f"  Max Latency:         {quetzalcore_stats['max_latency']:.2f} ms")
    print(f"  Throughput:          {quetzalcore_stats['throughput']:.0f} req/s")
    print(f"  Avg Overhead:        {quetzalcore_stats['avg_overhead']:.0f} bytes/msg")
    print(f"  Total Duration:      {quetzalcore_stats['duration']:.2f} s")
    ...
    print("\n🏆 IMPROVEMENT:")
    latency_improvement = rest_stats['avg_latency'] / quetzalcore_stats['avg_latency']
    throughput_improvement = quetzalcore_stats['throughput'] / rest_stats['throughput']
    overhead_reduction = (rest_stats['avg_overhead'] - quetzalcore_stats['avg_overhead']) / rest_stats['avg_overhead'] * 100
...
async def main():
    ...
    try:
        ...
        print("   Make sure the server is running with: ./deploy-quetzalcore.sh")
        return
    
    # Run benchmarks
    quetzalcore_stats = await benchmark_quetzalcore(NUM_REQUESTS)
    rest_stats = benchmark_rest(NUM_REQUESTS)
    
    # Print results
    print_comparison(quetzalcore_stats, rest_stats)

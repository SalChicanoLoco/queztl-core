...
**Queztl Cloud Desktop**: Self-hosted remote desktop platform that delivers full Windows/Linux desktops to any device via browser or client. **Zero Microsoft dependencies.**

```
User's Browser/Phone          →    Queztl OS (Your Hardware)    →    Virtual Desktops
   ┌────────────┐                    ┌─────────────────┐              ┌──────────┐
   │  Phone     │ ──────────────────→│ Queztl Hypervisor│─────────────→│ Win 11   │
...
```
1. Open phone browser → queztl.company.com
...
**Setup:**
```bash
# Install Guacamole on Queztl OS
docker-compose up guacamole
# Configure RDP to Windows VMs
# Users access via https://queztl.com/desktop
```
...
```bash
✅ Queztl OS running (we have this!)
...
10 servers = 300 users (distributed Queztl cluster)
...
```bash
# On Queztl OS
./queztl-hypervisor create windows11 \
  --memory 4096 \
  --cpus 2 \
  --disk 40G
...
**Queztl Cloud Desktop Platform** becomes the **open-source alternative** to:
- Microsoft Azure Virtual Desktop
- Amazon WorkSpaces
...
1. Create one Windows 11 VM on Queztl
2. Install Guacamole

...
RUNNER_IMAGE = os.getenv("RUNNER_IMAGE", "quetzalcore-training-runner:latest")
...
async def list_runners():
    ...
    try:
        containers = docker_client.containers.list(
            all=True,
            filters={"name": "quetzalcore-runner"}
        )
...
async def spawn_runner(count: int = 1):
    ...
    for i in range(count):
        runner_id = f"quetzalcore-runner-{len(active_runners) + i + 1}"
...
async def startup_event():
    ...
    try:
        containers = docker_client.containers.list(filters={"name": "quetzalcore-runner"})
